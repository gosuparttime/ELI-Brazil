...
add_filter('admin_footer_text', 'eli_custom_admin_footer');
function eli_custom_admin_footer() {
...
// Thumbnail sizes
add_image_size( 'carousel', 570, 350, true);
add_image_size( 'home-slide', 360, 480, true);
add_image_size( 'small-feature', 365, 225, true );
/* 
...
function eli_register_sidebars() {
	...
	register_sidebar(array(
      'id' => 'sidebar-2',
      'name' => 'Second Page Sidebar',
	  'description' => 'Sidebar item for secondary pages',
      'before_widget' => '<div id="%1$s" class="widget %2$s">',
      'after_widget' => '</div>',
      'before_title' => '<h3 class="widgettitle">',
      'after_title' => '</h3>',
...
// Comment Layout
function eli_comments($comment, $args, $depth) {
		...
		<article id="comment-<?php comment_ID(); ?>" class="clearfix">
			<div class="comment-author vcard row-fluid clearfix">
				...
				<div class="span10 comment-text">
					<?php printf(__('<h4>%s</h4>','eli-theme'), get_comment_author_link()) ?>
					<?php edit_comment_link(__('Edit','eli-theme'),'<span class="edit-comment btn btn-small btn-info"><i class="icon-white icon-pencil"></i>','</span>') ?>
                    
                    <?php if ($comment->comment_approved == '0') : ?>
       					<div class="alert-message success">
          				<p><?php _e('Your comment is awaiting moderation.','eli-theme') ?></p>
          				</div>
					<?php endif; ?>
...
// Search Form
function eli_wpsearch($form) {
    $form = '<form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" >
    <label class="screen-reader-text" for="s">' . __('Search for:', 'eli-theme') . '</label>
    <input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="Search the Site..." />
    <input type="submit" id="searchsubmit" value="'. esc_attr__('Search','eli-theme') .'" />
    </form>';
    return $form;
} // don't remove this bracket!
...
function custom_password_form() {
	...
	' . __( "<p>This post is password protected. To view it please enter your password below:</p>" ,'eli-theme') . '
	<label for="' . $label . '">' . __( "Password:" ,'eli-theme') . ' </label><div class="input-append"><input name="post_password" id="' . $label . '" type="password" size="20" /><input type="submit" name="Submit" class="btn btn-primary" value="' . esc_attr__( "Submit",'eli-theme' ) . '" /></div>
	</form></div>
	';
	return $o;
}
...
function theme_js(){
  ...
  wp_register_script('retina', get_template_directory_uri().'/library/js/libs/retina.js');

  // wp_enqueue_script('less', array(''), '1.3.0', true);
  wp_enqueue_script('jquery');
  wp_enqueue_script('bootstrap', array('jQuery'), '2.3.1', true);
  ...
  wp_enqueue_script('scripts', array('jQuery'), '1.0', true);
  wp_enqueue_script('modernizr', array('jQuery'), '2.6.1', true);
  wp_enqueue_script('retina', array('jQuery'), '0.0.2', true);
}
